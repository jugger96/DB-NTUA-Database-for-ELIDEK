-- Calc age
select *, round(datediff(curdate(),birth_date)/365, 0) as Age from researcher;
...
-- Query 3.7
create view view37 as (select executive_ID, first_name, last_name, name as Company_Name, ta.id as company_id, sum(funding) as total_funding from
...
-- Query 3.6
create view view36 as (select id, first_name, last_name, count(*) as projects from (select * from (select r.id, r.first_name, r.last_name, r.birth_date 
